...
def train_state(
    ...
    policy = MLPPolicy(
        state_dim=state_dim,
        action_dim=action_dim,
        hidden_dims=cfg.get("hidden_dims", [256, 256, 256]),
    ).to(device)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in train_loader:
            ...
            if global_step % log_every == 0:
                lr = optimizer.param_groups[0]["lr"]
                logger.info("epoch %d  step %d  loss %.4f  lr %.2e", epoch, global_step, loss.item(), lr)
                wandb_log(
                    wandb_run,
                    {"train/loss": loss.item(), "learning_rate": lr, "epoch": epoch, "global_step": global_step},
                    step=global_step,
                )
        ...
        val_loss = _validate_state(policy, val_loader, criterion, device)
        if val_loss is not None:
            logger.info("Epoch %d  val_loss %.4f", epoch, val_loss)
            wandb_log(wandb_run, {"val/loss": val_loss, "epoch": epoch}, step=global_step)
...
def _validate_state(
    ...
    with torch.no_grad():
        for batch in loader:
            pred = policy(batch["state"].to(device))
            total_loss += criterion(pred, batch["action"].to(device)).item()
    return total_loss / len(loader)
