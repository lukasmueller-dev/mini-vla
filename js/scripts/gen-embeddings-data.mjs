...
await writeFile(path.join(ROOT, "assets/embeddings-50d.bin"), bin);
await writeFile(path.join(ROOT, "assets/vocab.txt"), words.join("\n"));
...
const gen = `// GENERATED by scripts/gen-embeddings-data.mjs — do not edit.
...
`;
await writeFile(path.join(ROOT, "src/vocab.gen.ts"), gen);

console.log(
  `wrote assets/embeddings-50d.bin (${(bin.length / 1024).toFixed(0)}KB), ` +
    `assets/vocab.txt, src/vocab.gen.ts (${Object.keys(core).length} core words)`
);
