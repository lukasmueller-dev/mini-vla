...
import { extractPolicyWeights, type PolicyCheckpoint } from "./policy-weights";
import {
  COLORS,
  LABEL_TOKEN_IDS,
  MAX_SEQ_LEN,
...
// PredictResult / DecodedCommand now live in ./infer (shared with the replay
// fallback, trainer.replay.ts); re-exported here so the public trainer.core
// surface (and its importers: trainer.ts, trainer.worker.ts, rollout.ts) is
// unchanged.
export type { DecodedCommand, PredictResult } from "./infer";
...
export class VLATrainerCore {
  ...
  /** When set (VLATrainer sets it iff replayFallback is on), a thrown gradient
      step surfaces reason "train" IMMEDIATELY instead of retrying on the cpu
      backend — so the outer replay fallback takes over. The replay (a pretrained
      policy) beats slow cpu training (~5min), and the cpu restart would escape
      the load watchdog (disarmed after the first batch), so it must NOT run when
      a replay is standing by. Off ⇒ the original webgl→cpu recovery. */
  skipCpuFallback = false;
  ...
  private runId = 0;
  /** Lazily-created scene + thumbnail canvases for the silhouette render
      pipeline (the render code itself lives in ./infer, shared with replay). */
  private canvases: RenderCanvases | null = null;
  ...
  /** Lazily create (once) and return the render canvases (see ./infer). */
  private cv(): RenderCanvases {
    return (this.canvases ??= makeRenderCanvases());
  }
  ...
  async start(onUpdate?: () => void, assetBase?: string): Promise<void> {
    ...
    try {
      ...
      // Recover by falling back to the cpu backend once (this small CNN trains
      // fine without a GPU, just slower) and retrying from a clean model — UNLESS
      // a replay fallback is standing by (skipCpuFallback), in which case surface
      // reason "train" so VLATrainer swaps to the replay instead: it beats slow
      // cpu training AND the cpu restart would otherwise escape the load watchdog
      // (disarmed after the first batch) and drag on for minutes.
      if (!this.running || this.runId !== myRun) return;
      console.error("VLA trainer step failed", err);
      if (!this.skipCpuFallback && this.tf.getBackend() !== "cpu") {
   ...
   * the viz twin, return the predicted ABSOLUTE target joint angles plus the
   * spatial-attention readout (not a delta — the caller subtracts its own known
   ...
   * (Render + forward live in ./infer, shared with the replay fallback.)
   */
  predictTarget(
    a1: number,
    a2: number,
    ...
    return inferTarget(this.tf!, this.models!, this.cv(), a1, a2, tokens, layout, carry);
  }
  ...
  /** Snapshot the current TRAINABLE policy weights in the compact replay
      checkpoint format (see policy-weights.ts) — used ONLY by the dev capture
      script (js/capture) to record checkpoints along a real training run for
      the replay fallback. Null until the first batch has built the model. */
  exportTrainableWeights(): PolicyCheckpoint | null {
    if (!this.models) return null;
    return extractPolicyWeights(this.models.model);
  }
  ...
  decodeCommand(tokens: number[]): DecodedCommand | null {
    if (!this.ready) return null;
    return decodeColor(this.tf!, this.models!, tokens);
  }

  /**
   * The language encoder's per-token ATTENTION weights — the live per-chip
   * bars, recomputed CPU-side from the linear conv scorer + the frozen
   * embedding table (no forward pass; see ./infer tokenAttention).
   */
  attentionWeights(tokens: number[]): number[] | null {
    if (!this.ready) return null;
    return tokenAttention(this.models!, tokens);
  }
