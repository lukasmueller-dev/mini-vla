# mini-vla · js/ (the in-browser artifact)

> **This is the ported TensorFlow.js output artifact for the portfolio.** The
> model is developed in Python at the repo root (`mini_vla/`, the source of
> truth — see the [root README](../README.md)); this `js/` tree re-expresses the
> same architecture + task in TF.js so it can train *live in the browser*. It is
> regenerated from the Python via the Python→JS port skill (architecture + task
> parity, not weights — the browser demo retrains from scratch). File paths below
> are under `js/src/`; the npm scripts run from the **repo root**.
...
**ESM-only TypeScript source package**. It ships the framework-free `js/src/` and
the pretrained embedding assets (`assets/`, shared with the Python package); a
host (e.g. a Next.js site) transpiles the source and renders whatever UI it
likes on top of the data the package exposes.
...
the grammar, vocab size, or quantization changes; they're committed). The
generator emits the metadata to **both** `js/src/vocab.gen.ts` and the Python
mirror `mini_vla/vocab_gen.py` in lockstep, and reads the shared word inventory
from `assets/grammar.json`. A host serves the assets wherever
`loadEmbeddings({ assetBase })` points — default `/vla`.
